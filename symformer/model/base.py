...
class TransformerBase(tf.keras.Model):
    ...
    def contrastive_loss(self, expr_ft, data_ft):
        print("tf.norm(expr_ft, ord=2, axis=-1):", tf.norm(expr_ft, ord=2, axis=-1).shape)
        print("expr_ft:", expr_ft.shape)
        print("tf.norm(data_ft, ord=2, axis=-1):", tf.norm(data_ft, ord=2, axis=-1).shape)
        print("data_ft:", data_ft.shape)
        expr_ft_norm = expr_ft/(tf.expand_dims(tf.norm(expr_ft, ord=2, axis=-1), axis=-1)+1e-5)
        data_ft_norm = data_ft/(tf.expand_dims(tf.norm(data_ft, ord=2, axis=-1), axis=-1)+1e-5)

        sim_d2e = expr_ft_norm @ tf.transpose(data_ft_norm) / 0.07
        sim_e2d = data_ft_norm @ tf.transpose(expr_ft_norm) / 0.07

        sim_targets = tf.linalg.set_diag(tf.zeros_like(sim_d2e), tf.ones_like(tf.linalg.diag_part(sim_d2e)))
        
        loss_d2e = -tf.reduce_mean(tf.nn.log_softmax(sim_d2e, axis=-1)*sim_targets, axis=1)
        loss_e2d = -tf.reduce_mean(tf.nn.log_softmax(sim_e2d, axis=-1)*sim_targets, axis=1)

        return (loss_d2e + loss_e2d) / 2
