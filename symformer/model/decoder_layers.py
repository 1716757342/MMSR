...
from .config import DecoderConfig, DecoderWithRegressionConfig, EncoderExprConfig
...
# ljy: this decoder could be the multimodal encoder
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask):
        ...
        # x: label, the pre-order traversal
        # print("x:", x.shape)
        ...
        # print("attn_weights_block1:", attn_weights_block1.shape)
        ...
        # print("attn_weights_block2:", attn_weights_block2.shape)
...
class EncoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads, dff, rate=0.1):
        ...
        # self.mha1 = MHA(num_heads, d_model)
        self.attn_layer = tf.keras.layers.Attention(use_scale=True, dropout=rate)
...
class DecoderWithRegressionAsSeq(tf.keras.layers.Layer):
    def __init__(self, cfg: DecoderWithRegressionConfig, enc_cfg: EncoderExprConfig, vocab_len):
        super().__init__()
        ...
        self.expr_encoder = [
            EncoderLayer(enc_cfg.d_model, enc_cfg.num_heads,
                         enc_cfg.d_ff, enc_cfg.dropout_rate)
    ...
    def call(self, x, enc_output, training, look_ahead_mask, reg_tar):
        ...
        for i, layer in enumerate(self.expr_encoder):
            enc = layer(enc, training, look_ahead_mask)

        for i, layer in enumerate(self.dec_layers):
            dec, block1, block2 = layer(dec, enc_output, training, look_ahead_mask)
            attention_weights[f"decoder_layer{i + 1}_block1"] = block1
            attention_weights[f"decoder_layer{i + 1}_block2"] = block2

        # x.shape == (batch_size, target_seq_len, d_model)
        return enc, dec, attention_weights
