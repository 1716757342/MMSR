...
class TransformerWithRegressionAsSeq(TransformerBase):
    def __init__(self, cfg, tokenizer, input_regularizer, strategy):
        ...
        self.decoder = DecoderWithRegressionAsSeq(
            self.cfg.decoder_config, self.cfg.encoder_expr, len(self.tokenizer.vocab)
        )
    ...
    def call(self, inputs, training):
        ...
        # )
        expr_enc, dec_output, attention_weights = self.decoder(
            tar, enc_output, training, look_ahead_mask, reg_tar
        )
        ...
        return enc_output, expr_enc, final_output, reg_head_output, attention_weights
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # expr_ft: expression feature
            data_ft, expr_ft, predictions, regression_predictions, _ = self(
                (points, symbolic_input, regression_input), training=True
            )
            contrastive_loss = self.contrastive_loss(tf.reduce_sum(expr_ft, axis=1), 
                                                     tf.reduce_sum(data_ft, axis=1))
            # print("contrastive_loss:", contrastive_loss.numpy())
            classification_loss = self.classification_loss(predictions, symbolic_tar)
            # print("classification_loss:", classification_loss.numpy())
            regression_loss = self.regression_loss(
                regression_target, regression_predictions, self.min_regression_loss
            )
            # print("regression_loss:", regression_loss.numpy())
